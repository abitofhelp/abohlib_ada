...
--   ==========================================================================
   ...
   begin
      return Assert_True (Parallel_Stage.Is_Initialized, "Parallel stage should be initialized");
   end Test_Parallel_Stage_Creation;

   function Test_Parallel_Basic_Processing return Void_Result.Result is
      Config : constant Test_Config := (Multiplier => 4, Fail_On_Negative => False);
      Parallel_Stage : Parallel_Integer_Doubler.Pipeline_Stage :=
        Parallel_Integer_Doubler.Create (Config);
      Result : constant Parallel_Stage_Result.Result := Parallel_Stage.Process (7);
   begin
      if not Parallel_Stage_Result.Is_Ok (Result) then
         return Assert_True (False, "Parallel processing should succeed");
      end if;

      return Assert_Equal (28, Parallel_Stage_Result.Get_Ok (Result), "7 * 4 = 28");
